...
async def transcribe_recv(file: Path):
    ...
    # 得到文件名
    output_dir = Config.user_data_dir
    makedirs(output_dir, exist_ok=True)

    # 使用原文件名但保存到 data/user 目录
    file_name = file.name
    json_filename = output_dir / Path(file_name).with_suffix(".json")
    txt_filename = output_dir / Path(file_name).with_suffix(".txt")
    merge_filename = output_dir / Path(file_name).with_suffix(".merge.txt")
